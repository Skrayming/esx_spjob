...
AddEventHandler("AppelemsTropBien", function(coords, id)
	...
	AppelCoords = coords
	AppelID = id
	ESX.ShowAdvancedNotification("ems", "~b~Demande de ems", "Quelqu'un à besoin d'un sapeur pompier !\n~g~Y~w~ pour prendre l'appel\n~r~X~w~ pour refuser", "CHAR_CALL911", 8)
end)
     ...
     while true do
		...
		if AppelEnAttente then
			if IsControlJustPressed(1, 246) and AppelEnAttente then
				if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'sp' then
...
AddEventHandler("emsAppelPris", function(Xid, XAppelCoords)
	ESX.ShowAdvancedNotification("ems", "~b~Demande de sapeur pompier", "Vous avez pris l'appel, suivez la route GPS.", "CHAR_CALL911", 8)   
...
function afficherTextVolant(XAcoords, XAid)
	...
	AddTextComponentSubstringPlayerName("~b~Appel sapeur pompier")
     ...
     while rea do
          if GetDistanceBetweenCoords(XAcoords, GetEntityCoords(GetPlayerPed(-1))) < 10.0 then
               ...
               if IsControlJustReleased(0, 38) then
				...
				TriggerServerEvent('esx_spjob:removeItem', 'medikit')
				TriggerServerEvent('esx_spjob:revive', XAid)
